    ...
    try {
      if (dataType === "donors") {
        if (!row.name || typeof row.name !== "string" || !row.name.trim()) {
          results.push({ index: i, status: "failed", reason: "Missing required field: name", id: null });
          continue;
        }
        ...
        if (normEmail) {
          ...
          const matches = await db.select().from(donorsTable).where(sql`lower(${donorsTable.email}) = ${normEmail}`);
          existing = matches[0] ?? null;
        }

        if (existing) {
          if (duplicateStrategy === "skip") {
            results.push({ index: i, status: "skipped", reason: `Duplicate (matched by email: ${normEmail})`, id: existing.id });
            continue;
          }
          if (duplicateStrategy === "update") {
            ...
            await db.update(donorsTable).set(updateData).where(eq(donorsTable.id, existing.id));
            results.push({ index: i, status: "updated", reason: null, id: existing.id });
            continue;
          }
        }

        const [created] = await db.insert(donorsTable).values({
          name: row.name.trim(),
          email: normEmail,
        ...
        results.push({ index: i, status: "created", reason: null, id: created.id });
        ...
        if (!date) {
          results.push({ index: i, status: "failed", reason: "Missing or invalid date", id: null });
          continue;
        }
        if (!row.location || typeof row.location !== "string" || !row.location.trim()) {
          results.push({ index: i, status: "failed", reason: "Missing required field: location", id: null });
          continue;
        }
        if (!row.eventType || typeof row.eventType !== "string" || !row.eventType.trim()) {
          results.push({ index: i, status: "failed", reason: "Missing required field: eventType", id: null });
          continue;
        }

        // Case-insensitive event name match
        const normName = row.name.trim().toLowerCase();
        const matches = await db.select().from(eventsTable).where(
        ...
        if (existing) {
          if (duplicateStrategy === "skip") {
            results.push({ index: i, status: "skipped", reason: `Duplicate (matched by name+date)`, id: existing.id });
            continue;
          }
          if (duplicateStrategy === "update") {
            ...
            if (row.notes) updateData.notes = String(row.notes);
            await db.update(eventsTable).set(updateData).where(eq(eventsTable.id, existing.id));
            results.push({ index: i, status: "updated", reason: null, id: existing.id });
            continue;
          }
        }
        ...
        if (donorId == null || !Number.isInteger(donorId)) {
          results.push({ index: i, status: "failed", reason: "Missing or invalid donorId", id: null });
          continue;
        }
        const donor = await db.select().from(donorsTable).where(eq(donorsTable.id, donorId));
        if (!donor[0]) {
          results.push({ index: i, status: "failed", reason: `Donor #${donorId} not found`, id: null });
          continue;
        }
        const amount = parseAmount(row.amount);
        if (amount == null || amount <= 0) {
          results.push({ index: i, status: "failed", reason: "Missing or invalid amount", id: null });
          continue;
        }
        const date = normalizeDate(row.date);
        if (!date) {
        ...
        if (row.eventId != null && row.eventId !== "") {
          ...
          if (n != null && Number.isInteger(n)) {
            const ev = await db.select().from(eventsTable).where(eq(eventsTable.id, n));
            if (!ev[0]) {
              results.push({ index: i, status: "failed", reason: `Event #${n} not found`, id: null });
              continue;
            }
            eventId = n;
          }
        ...
        const [created] = await db.insert(donationsTable).values({
          donorId,
          eventId,
          date,
        ...
        if (eventId == null || !Number.isInteger(eventId)) {
          results.push({ index: i, status: "failed", reason: "Missing or invalid eventId", id: null });
          continue;
        }
        const event = await db.select().from(eventsTable).where(eq(eventsTable.id, eventId));
        if (!event[0]) {
          results.push({ index: i, status: "failed", reason: `Event #${eventId} not found`, id: null });
          continue;
        }
        const amount = parseAmount(row.amount);
        if (amount == null || amount <= 0) {
        ...
        if (!row.paymentType || typeof row.paymentType !== "string") {
          results.push({ index: i, status: "failed", reason: "Missing required field: paymentType", id: null });
          continue;
        }
        ...
        if (!row.taskName || typeof row.taskName !== "string" || !row.taskName.trim()) {
          results.push({ index: i, status: "failed", reason: "Missing required field: taskName", id: null });
          continue;
        }
      ...
      } else if (dataType === "followups") {
        if (!row.taskType || typeof row.taskType !== "string" || !row.taskType.trim()) {
          results.push({ index: i, status: "failed", reason: "Missing required field: taskType", id: null });
          continue;
        }
        if (!row.recommendedAction || typeof row.recommendedAction !== "string" || !row.recommendedAction.trim()) {
          results.push({ index: i, status: "failed", reason: "Missing required field: recommendedAction", id: null });
          continue;
        }
        let eventId: number | null = null;
        ...
        if (row.eventId != null && row.eventId !== "") {
          ...
          if (n != null && Number.isInteger(n)) {
            const ev = await db.select().from(eventsTable).where(eq(eventsTable.id, n));
            if (!ev[0]) lookupErr = `Event #${n} not found`;
            else eventId = n;
          }
        }
        if (!lookupErr && row.donorId != null && row.donorId !== "") {
          const n = parseAmount(row.donorId);
          if (n != null && Number.isInteger(n)) {
            const d = await db.select().from(donorsTable).where(eq(donorsTable.id, n));
            if (!d[0]) lookupErr = `Donor #${n} not found`;
            else donorId = n;
          }
        }
        if (!lookupErr && row.attendeeId != null && row.attendeeId !== "") {
          const n = parseAmount(row.attendeeId);
          if (n != null && Number.isInteger(n)) {
            const a = await db.select().from(attendeesTable).where(eq(attendeesTable.id, n));
            if (!a[0]) lookupErr = `Attendee #${n} not found`;
            else attendeeId = n;
          }
        }
        if (lookupErr) {
          results.push({ index: i, status: "failed", reason: lookupErr, id: null });
          continue;
        }
      ...
      results.push({ index: i, status: "failed", reason: msg, id: null });
    }
  }
